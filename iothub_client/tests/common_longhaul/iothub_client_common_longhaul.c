...
#define TELEMETRY_MESSAGE_FORMAT "longhaul-tests:%s;message-id:%u;"
...
static int on_message_received(void* context, const char* data, size_t size)
{
    ...
    else
    {
        ...
        size_t message_id;
        char longhaul_tests_id[36];
        //TODO: continue from here: fix the message.
        if (sscanf(data, TELEMETRY_MESSAGE_FORMAT, longhaul_tests_id, &message_id) == 2 &&
            strcmp(longhaul_tests_id, iotHubLonghaul->test_id) == 0)
        {
            TELEMETRY_INFO info;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                time_t time_start_range = add_seconds(time(NULL), 0);
