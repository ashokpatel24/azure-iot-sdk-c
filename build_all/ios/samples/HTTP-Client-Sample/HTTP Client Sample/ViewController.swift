...
class ViewController: UIViewController {
    ...
    private let iotProtocol: IOTHUB_CLIENT_TRANSPORT_PROVIDER = HTTP_Protocol
    ...
    func sendMessage() {
        ...
        // This the message
        message = "iOS message " + String(cntSent)
    ...
    let myConnectionStatusCallback: IOTHUB_CLIENT_CONNECTION_STATUS_CALLBACK = { result, reason, userContext
        in
        
        var mySelf: ViewController = Unmanaged<ViewController>.fromOpaque(userContext!).takeUnretainedValue()
        
        print("Connection callback: ", result, reason)
    }
    ...
    let myReceiveMessageCallback: IOTHUB_CLIENT_MESSAGE_CALLBACK_ASYNC = { message, userContext in
        ...
        if rc == IOTHUB_MESSAGE_OK {
            ...
            mySelf.lblLastMsg.text = messageString
        }
        else {
    ...
    @IBAction func startSend(sender: UIButton!) {
        ...
        if (IOTHUB_CLIENT_OK != (IoTHubClient_LL_SetConnectionStatusCallback(iotHubClientHandle, myConnectionStatusCallback, that))) {
            showError(message: "Failed to establish connection callback", startState: true, stopState: false)
